...
class SaltonSea:
    ...
    def contourMap(self):
        x = np.random.normal(2., 1., (0,0))  # generate a random map
        exten = np.array([0, 23, 0, 23])

        fig = pl.figure()
        fig.canvas.set_window_title('Salton Sea Contour Map')

        ax = fig.add_subplot(111)
        ax.set_axis_bgcolor('#b1a688')

        im = pl.imshow(x,  origin='lower',extent=exten)

        #x and y coordinates of the contour map in a string form

        #outer contour to inner
        ...
        ax.add_patch(Polygon(h, fill=True, color="#0075e5"))
        ...
        ax.add_patch(Polygon(g, fill=True, color="#0068cc"))
        ...
        ax.add_patch(Polygon(f, fill=True, color="#005bb2"))
        ...
        ax.add_patch(Polygon(e, fill=True, color="#004e99"))
        ...
        ax.add_patch(Polygon(d, fill=True, color="#00417f"))
        ...
        ax.add_patch(Polygon(c, fill=True, color="#003466"))
        ...
        ax.add_patch(Polygon(b, fill=True, color='#00274c'))
        ...
        ax.add_patch(Polygon(a2, fill=True, color='#001a33'))
        ...
        ax.add_patch(Polygon(a1, fill=True, color='#001a33'))

        pl.title('Salton Sea Contour Map')
    ...
    def __init__(self, SaltonSea, title, inflow, **kwargs):
        ...
        while True:
            try:
                ...
                SaltonSea.numYears = int(input())
                SaltonSea.calculateEvaporation()
                break
            except ValueError:
